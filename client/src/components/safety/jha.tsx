...
export function SafetyJha() {
  ...
  const [genPlanDescription, setGenPlanDescription] = useState("");
    ...
    mutationFn: async () => {
      ...
      const res = await fetch("/api/safety/jha/generate", {
        ...
        body: JSON.stringify({
          ...
          dayId: activeDay?.id,
          planDescription: genPlanDescription || undefined,
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: "JHA generation failed" }));
        throw new Error(err.error || "JHA generation failed");
      }
      return res.json();
    },
    onSuccess: (data) => {
      ...
      <ScrollArea className="h-full">
        <div className="p-4 max-w-2xl mx-auto space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-white">Generate JHA</h2>
              <p className="text-sm text-muted-foreground">
                Describe the plan of the day and provide context to generate a comprehensive Job Hazard Analysis
              </p>
            </div>
          ...
          <Card className="bg-card border-border">
            <CardContent className="pt-4 space-y-4">
              <div>
                <Label className="text-xs">Plan of the Day Description</Label>
                <Textarea
                  placeholder="Describe today's dive plan, objectives, and any special considerations..."
                  rows={3}
                  value={genPlanDescription}
                  onChange={(e) => setGenPlanDescription(e.target.value)}
                />
              </div>
              <div>
              ...
              >
                {generateMutation.isPending ? "Generating..." : "Generate JHA"}
              </Button>
      ...
      <div className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Generated and manual JHA records</p>
          </div>
          {isSupervisor && (
            <Button
              size="sm"
        ...
        {isLoading ? (
          ...
          <Card className="bg-card border-border">
            <CardContent className="py-8 text-center text-muted-foreground">
              ...
              {isSupervisor && (
                <p className="text-xs mt-2">Click "Generate JHA" to create one.</p>
              )}
            </CardContent>
          </Card>
        ) : (
                ...
                <CardContent className="py-3 px-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {jha.aiGenerated && (
                        <Badge className="bg-purple-500/20 text-purple-400 text-xs">Generated</Badge>
                      )}
